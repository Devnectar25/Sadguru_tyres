...
export default function CatalogPage({
  ...
  // Filters state
  const [vehicleType, setVehicleType] = useState("All"); // "All", "Cars", "SUVs", "Bikes"
  ...
  const brands = useMemo(() => ["All", ...new Set(TYRES_DATA.map((t) => t.brand))], []);
  const tyreTypes = useMemo(() => ["All", ...new Set(TYRES_DATA.map((t) => t.tyreType))], []);
  const widths = useMemo(() => ["All", ...new Set(TYRES_DATA.map((t) => t.width))].sort(), []);
  const profiles = useMemo(() => ["All", ...new Set(TYRES_DATA.map((t) => t.profile))].sort(), []);
  const rimSizes = useMemo(() => ["All", ...new Set(TYRES_DATA.map((t) => t.rimSize))].sort((a, b) => Number(a) - Number(b)), []);
  const performanceLevels = useMemo(() => ["All", ...new Set(TYRES_DATA.map((t) => t.performanceLevel))], []);
  ...
  const filteredTyres = useMemo(() => {
    return TYRES_DATA.filter((tyre) => {
      ...
      // Price Filter (based on USD base price)
      ...
      // recommended (default: rating * review count)
    ...
    >
      <div className="container">
        ...
        <div style={{ marginBottom: "36px" }}>
          ...
          >
            Explore Our <span className="text-gradient-crimson">Tyres</span>
          </h1>
          <p
            style={{
              fontSize: "1.1rem",
          ...
          >
            Discover track-tested performance, all-weather grand touring comfort, and heavy-duty 4x4 engineering. Tailored fitments for sports cars, luxury SUVs, and superbikes.
          </p>
        </div>

        {/* Vehicle Type Selector Tabs (Cars / SUVs / Bikes) */}
        <div
          style={{
            display: "flex",
            alignItems: "center",
            ...
            return (
              <button
                ...
                style={{
                  ...
                  boxShadow: isSelected ? "0 4px 20px rgba(255, 255, 255, 0.25)" : "none",
                }}
              >
                {Icon && <Icon size={18} color={isSelected ? "#07080b" : "#ffffff"} />}
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "14px" }}>
            ...
            <div style={{ fontSize: "0.86rem", color: "var(--text-muted)" }}>
              Showing <strong style={{ color: "#ffffff" }}>{filteredTyres.length}</strong> tyres
            </div>
        ...
        {/* PRODUCT CARDS GRID (3 IN A LINE) */}
        <main>
          {filteredTyres.length === 0 ? (
            ...
            >
              ...
              <p style={{ color: "var(--text-dim)", maxWidth: "420px", margin: "0 auto 20px" }}>
                Try adjusting your search query or vehicle type filter to see available tyres.
              </p>
              <button onClick={handleResetFilters} className="btn btn-primary" style={{ padding: "10px 20px" }}>
                Reset Search
              </button>
            </div>
          ) : (
                ...
                {filteredTyres.map((tyre) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        <button
                          ...
                          style={{
                            ...
                            height: "36px",
                            borderRadius: "50%",
                            background: isWishlisted ? "rgba(255, 42, 42, 0.2)" : "rgba(10, 14, 20, 0.8)",
                            border: isWishlisted ? "1px solid var(--accent-crimson)" : "1px solid rgba(255, 255, 255, 0.15)",
                      ...
                      {/* Tyre Image Box - Large Studio Framing */}
                      <div
                      ...
                      >
                        ...
                        >
                          <span style={{ fontSize: "0.76rem", color: "var(--text-muted)", textTransform: "uppercase", fontWeight: "700", letterSpacing: "0.08em" }}>
                            {tyre.brand}
                          </span>
                        ...
                        >
                          <div>
                            ...
                            <div style={{ fontSize: "1.3rem", fontWeight: "700", color: "#ffffff", fontFamily: "var(--font-body)", letterSpacing: "normal" }}>
                              {displayPrice}
                            </div>
                          </div>
      ...
      <style>{`
        ...
        .product-card-hover:hover {
          ...
          box-shadow: 0 20px 40px -10px rgba(0, 0, 0, 0.8), 0 0 30px rgba(255, 42, 42, 0.3);
